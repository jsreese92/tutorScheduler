$(document).ready(function () {
	...
	//fill out the current schedule table if it's there
	$("table#tutor_schedule_result td").each(function(i, e) {
		var td = $(e);
		var name = td.attr('class');
		var dropdown = $("<select name="+name+"></select>");
		dropdown.append("<option>"+0+"</option><option>"+1+"</option><option>"+2+"</option>");
		dropdown.val(td.attr('data-status'));
		dropdown.hide();
